...
(function ($, undefined) {
    ...
    $(document).ready(function () {
        ...
        function formatDate(date) {
            ...
            var hours = Math.floor(diff / 3600000); // округлить diff до минут
            if (hours < 24) {
                return hours + ' ч. назад';
            }

            var days = Math.floor(diff / 86400000); // округлить diff до минут
            if (days < 24) {
        ...
        function getCommentItemData(action) {
            return {
                ...
                repo_name: action.repo.name.split('/')[1], // removing 'silverbulleters/'
            ...
            $.ajax({
                ...
                success: function (result) {
                    var min_date = moment().add('days', -MAX_OFFSET_DATE),
                        ...
                        events_by_days = {},
                        sparkline_data = [],
                        created_at_diff_max = today.diff(moment(result[result.length - 1].created_at), 'days'),
